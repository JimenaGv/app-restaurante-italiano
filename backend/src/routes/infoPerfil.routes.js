...
import InfoPerfil from '../models/infoPerfil.model.js'
...
perfilRouter.get('/:id', async (req, res) => {
  try {
    const perfil = await InfoPerfil.findById(req.params.id)
    if (!perfil) {
...
perfilRouter.put('/:id', async (req, res) => {
  try {
    const perfilActualizado = await InfoPerfil.findByIdAndUpdate(
      req.params.id,
      req.body,
